...
*we start by computing the "China Shock_k" for each industry in a region, and then we make compute the regional shock by taking the sum over all industries in the region (following the formula by Colantone & Stanig, AJPS, 2018)
...
They come from missing employment data. 
To see where, use: 
...
outreg2 using "Output/TABLE_P6.xls", excel replace  title("Effect of the China Shock (1995-2006) on the Post-Crisis (2014-2017) industry-region average TFP and Wage") addstat("Mean Dependent Var", M_tfp) addtext(Country FE, Yes, Industry FE, Yes) addnote("Robust Standard Errors") cttop(OLS) nocons
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("F-statistic instruments", F_weak) addtext(Country FE, Yes, Industry FE, Yes) cttop(First Stage) nocons
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("Mean Dependent Var", M_tfp) addtext(Country FE, Yes, Industry FE, Yes) cttop(Reduced Form) nocons
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("Mean Dependent Var", M_tfp) addtext(Country FE, Yes, Industry FE, Yes) cttop(Second Stage) nocons 
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("Mean Dependent Var", M_wages) addtext(Country FE, Yes, Industry FE, Yes) cttop(OLS) nocons  
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("Mean Dependent Var", M_wages) addtext(Country FE, Yes, Industry FE, Yes) cttop(Reduced Form) nocons
...
outreg2 using "Output/TABLE_P6.xls", excel append addstat("Mean Dependent Var", M_wages) addtext(Country FE, Yes, Industry FE, Yes) cttop(Second Stage) nocons

pwcorr Mean_tfp Mean_wages, star(.1)
...
*preliminary regression to get the weighted average of the radical right dummy in our sample using the intercept og the weighted OLS regression with no independent variables - best linear fit is the weighted average 
reg Radical_Right_Dummy [pweight=pspwght], cluster(nuts2)
*weighted average: 0.0397123
...
****** 	ROBUSTNESS CHECK ******
* We run a robustness check to make sure that our results are not driven by a few regions with large historical support for Lega Nord - Lombardy and Veneto (note, this would exacerbate our small-sample bias)
